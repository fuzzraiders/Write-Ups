...
```
ffuf              → Fuzzing for subdomains and hidden directories
SecLists          → Wordlists used for discovery and credential brute-forcing
Web Browser       → Manual inspection of source code and API responses
Burp Suite/Curl   → (Optional) Testing API endpoints and IDOR parameters
```
